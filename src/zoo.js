...
/* Explicação Requisito 5: V1: Busca com o .find se o id do
employees.manager está incluso no id (parâmetro),
se sim retorna verdadeiro, senão falso (para não retornar undefined).
V2: Coleta os dados dos gerentes e valida com um if
se o id (parâmetro) é igual a um id de gerente */
function isManager(id) {
  /* VERSÃO 1 */
  const isManager = employees.find(employee => employee.managers.includes(id)) ? true : false;
  return isManager;
  /* VERSÃO 2
...
/* Explicação Requisito 13: (resolução feita no plantão do Ícaro) */
function employeeCoverage(idOrName) {
  ...
  filteredEmployees.forEach(employee => {
     const mappedAnimals = employee.responsibleFor.map((animalId) => {
        const animalFound = animals.find ((animal) => animal.id === animalId).name;
        return animalFound;
      });
